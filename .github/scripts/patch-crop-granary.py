...
start = s.index("function cutGrass(")
end = s.index("function targetStillNeedsPlanting", start)
new_cut_grass = '''function cellNeedsHarvest(index) {
...
'''
s = s[:start] + new_cut_grass + s[end:]

target_old = "return Boolean(cuttable[index] && !cut[index]);"
assert s.count(target_old) == 1, f"targetStillNeedsMowing match count {s.count(target_old)}"
s = s.replace(target_old, "return cellNeedsHarvest(index);", 1)

candidate_old = "if (!cuttable[index] || cut[index]) continue;"
assert s.count(candidate_old) == 2, f"harvester candidate match count {s.count(candidate_old)}"
s = s.replace(candidate_old, "if (!cellNeedsHarvest(index)) continue;")
...
s = s.replace(
    'ui.status.textContent = `Auto · hauling ${grainCargoKg(mower).toFixed(1)} kg to granary`;',
    'ui.status.textContent = `Auto · hauling ${grainCargoKg(mower).toFixed(1)} kg crop to granary`;',
)

assert "grainCargoCells = (mowingAgent.grainCargoCells || 0) + newlyCut" not in s
assert "function cellNeedsHarvest(index)" in s
assert "harvestedCropCells" in s
assert s.count("if (!cellNeedsHarvest(index)) continue;") >= 3

game.write_text(s)

index = Path("index.html")
html = index.read_text()
html = html.replace(">TRACTOR · 5K</button>", ">TRACTOR · 5,000 PTS</button>")
